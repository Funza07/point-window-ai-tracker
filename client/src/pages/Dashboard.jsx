...
export default function Dashboard({ lib, setLib, setPage, setDetailTitle, onAdd, onOpenLink, isMobile = false }) {
  ...
  const hero = catalog[0] || trending[0] || mockTitles[0];
  const continueItems = lib.slice(0, 4).map((item) => ({ item, title: resolveLibraryTitle(item) })).filter((x) => x.title);
  const heroItems = (catalog.length ? catalog : trending).slice(0, 5).map((t) => ({
    ...t,
    banner: t?.banner || t?.cover || FALLBACK_IMAGE,
    cover: t?.cover || t?.banner || FALLBACK_IMAGE,
    title: t?.title || "Untitled",
    synopsis: t?.synopsis || "No synopsis available.",
      ...
      {!search && (
        isMobile ? (
          <div style={{ margin: "0 -14px" }}>
            <HeroCarousel items={heroItems} onView={onView} isMobile typeColor={typeColor} Badge={Badge} Btn={Btn} />
          </div>
        ) : (
          <div className="hero-banner" style={{ borderRadius: 24, overflow: "hidden", position: "relative", minHeight: 300, cursor: "pointer" }} onClick={() => onView(hero)}>
            <img src={hero?.banner || hero?.cover || FALLBACK_IMAGE} alt={hero?.title || "Untitled"} style={{ width: "100%", height: 340, objectFit: "cover" }} className="hero-img" />
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "32px 32px 28px" }}>
              ...
              <h1 style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 40, fontWeight: 900, color: "#f0ebff", margin: "0 0 6px", lineHeight: 1.05 }}>{hero?.title || "Untitled"}</h1>
              <p style={{ fontSize: 12, color: "#7a6b84", margin: "0 0 10px" }}>{hero?.alt || "-"} - {hero?.year || "-"} - {hero?.type || "Unknown"}</p>
              <p style={{ fontSize: 13, color: "#c4b5d0", maxWidth: 480, margin: "0 0 18px", lineHeight: 1.7 }}>{String(hero?.synopsis || "No synopsis available.").slice(0, 120)}...</p>
          ...
          {isMobile ? (
            <HScrollRow gap={12}>
              {continueItems.map(({ item, title }, i) => (
                <GlassCard key={title.id} style={{ padding: 14, display: "flex", gap: 12, minWidth: 260, flexShrink: 0, scrollSnapAlign: "start" }} delay={i * 60 + 100}>
                  <img src={title?.cover || title?.banner || FALLBACK_IMAGE} alt={title?.title || "Untitled"} style={{ width: 52, height: 72, objectFit: "cover", borderRadius: 10, flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 800, fontSize: 15, color: "#f0ebff", margin: "0 0 2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{title?.title || "Untitled"}</p>
                    <Badge color={typeColor(title?.type)}>{title?.type || "Unknown"}</Badge>
      ...
      {!search && (
        <section>
          ...
          {isMobile ? (
            <HScrollRow gap={10}>
              {trending.map((t, i) => (
                <GlassCard key={t.id} onClick={() => onView(t)} style={{ padding: "12px 10px 10px", textAlign: "center", minWidth: 96, flexShrink: 0, scrollSnapAlign: "start" }} delay={i * 50 + 200}>
                  ...
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#f0ebff", margin: "0 0 4px", lineHeight: 1.2 }}>{t?.title || "Untitled"}</p>
                  <p style={{ fontSize: 9, color: "#7a6b84", margin: 0 }}>{t?.type || "Unknown"}</p>
                </GlassCard>
              ))}
            </HScrollRow>
          ) : (
      ...
      {!search && (
        <section>
          ...
          {isMobile ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(155px, 1fr))", gap: 14 }}>
              {catalog.slice(4, 8).map((t, i) => <TitleCard key={t.id} title={{ ...t, cover: t?.cover || t?.banner || FALLBACK_IMAGE, genres: Array.isArray(t?.genres) ? t.genres : [] }} lib={lib} onAdd={handleAdd} onView={onView} delay={i * 60 + 250} />)}
            </div>
          )}
        </section>
      )}
