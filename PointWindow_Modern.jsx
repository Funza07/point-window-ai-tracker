...
function AIAssistant({ lib }) {
  ...
  const send = async (content) => {
    ...
    setLoading(true);
    try {
      const libraryContext = recentTitles.length ? `User's library includes: ${recentTitles.join(", ")}.` : "User has no titles in library yet.";
      const systemPrompt = `You are Aetheris, an AI companion for anime, manga, and manhwa fans. You are enthusiastic, knowledgeable, and speak with otaku energy. Keep responses concise (2-4 sentences). Spoiler-free mode is ${spoiler ? "ENABLED — never spoil plot twists" : "disabled"}. ${libraryContext}`;
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000, system: systemPrompt, messages: newMessages.map(m => ({ role: m.role === "ai" ? "assistant" : "user", content: m.content })) })
      });
